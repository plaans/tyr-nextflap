...
// Solves the planning task in anytime mode, returning all improving solutions
py::list solve_anytime(py::bool_ durativePlan) {
    py::list solutions;
    parsedTask->startTime = clock();
                        ...
                        if (solution != nullptr) {
                            ...
                            if (checker.checkPlan(solution, true, &cvarValues)) {
                                ...
                                if (solutionMakespan < bestMakespan ||
                                    ...
                                    // Add solution to list with quality metrics
                                    py::dict sol_dict;
                                    sol_dict["plan"] = PrintPlan::print(solution, &cvarValues, durativePlan);
                                    sol_dict["makespan"] = solutionMakespan;
                                    sol_dict["steps"] = solution->g;
                                    sol_dict["time"] = parsedTask->ellapsedTime();
                                    solutions.append(sol_dict);
                                }
                            }
                        }
                        // Check timeout within the loop
...
PYBIND11_MODULE(nextflap, m) {
    ...
    m.def("solve_anytime", &solve_anytime, "Solve the planning task in anytime mode");
}
